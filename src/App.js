...
// Use an environment variable for the API base URL. Fallback to localhost if not set.
const API_BASE_URL = process.env.REACT_APP_API_BASE_URL || 'http://localhost:8000';

function App() {
  const [sessionId, setSessionId] = useState('');
  const [step, setStep] = useState('input'); // steps: input, loading, summary, processingDecision, result
  ...
  // Call the analyze API when a token address is submitted.
...
const handleDecision = async (decision) => {
  ...
  try {
    ...
    const payload =
      decision === 'yes'
        ? { decision: decision, session_id: sessionId }
        : { session_id: sessionId };
    ...
    // If the status code is not in the 200 series, show the Base network error message.
    if (!response.ok) {
      setBuyResult('Please switch to Base network.');
      setStep('result: error');
      return;
    }

    // If response is OK, proceed as usual.
    const data = await response.json();
    setBuyResult(data.message || 'Operation successful.');
    setStep('result');
  } catch (error) {
    console.error('Error processing decision:', error);
    setBuyResult('An unexpected error occurred.');
    setStep('result');
  }
};
